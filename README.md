...
24개월 상품은 최소 30% 할인 규칙을 따릅니다.
25% 선택 시 실제 30%가 적용되며, 36% / 42.5% / 43%는 선택한 할인율이 그대로 적용됩니다.


## 서비스 시작일 / 환불 요청일 직접 수정

환불 계산의 `서비스 시작일`과 `환불 요청일`은 `YYYY-MM-DD` 형식의 텍스트 입력으로 제공됩니다.
마우스로 원하는 위치를 클릭해 연도, 월, 일을 직접 수정할 수 있습니다.

예: `2026-09-10`

숫자 8자리(`20260910`)로 입력한 뒤 입력칸 밖을 클릭하면 `2026-09-10` 형식으로 자동 정리됩니다.
...
`start-date`와 `refund-date`는 두 가지 방식으로 입력할 수 있습니다.

- `2026-10-10`처럼 직접 입력 또는 복사/붙여넣기
- 입력칸 오른쪽의 달력 버튼을 클릭해 날짜 선택

달력에서 날짜를 선택하면 `YYYY-MM-DD` 형식으로 자동 입력됩니다.
